...
| 32 | 1 | Explanation service, auditable, plain language, on demand (T10 test passing). Explanations are **provenance-first**: every sentence is a claim carrying its citation (`record:<id>`, `turn:<id>:decision.<key>`, concern/authority events) and is rendered from it, so text and citation cannot drift. `beanie/faithfulness.py` (+ `--audit-explanations`) runs the §9.9 protocol over every recorded turn: citations must resolve, asserted values must equal the source, every value the sentence shows must match, every material input (the question, the depth, an escalation, each pre-flight concern, the confidence weights, the residual, the skill an action ran, the authority state that governed it) must be cited, and the summary must come from a claim. Turns with no decision trace are reported as unaudited rather than passed. The boundary is stated where the claim is made: the audit proves the explanation matches the *recorded trace*; whether the trace captured everything the substrate did internally stays open (ARCHITECTURE §9.9). Live: 93 suite turns + the 34-turn demo, 0 violations, 0 unaudited the overseer day-digest extends this to ACTIVITY: 'what did we do today/yesterday' counts the trace per kind (empty day honestly stated), 'what are you working on' names the live queue (asks ×count with grant sentences, paused takeover + 'continue', parked incubator items) |
...
| 44–45 | 1 | `FileIndex` (Everything-class: incremental multi-root, kind hints, fuzzy near-spelling, citable why, honest-zero) + the local-first media flow through the gate: found→permission→launch, missing→YouTube top result *labeled* as such; **the media trail** walks ranked alternatives conversationally ("no, the other one" → next ranked match, position stated, exhaustion announced with the streaming door open — never a silent wrap); index tests + end-to-end decision-flow tests + trail tests + the `media_intent` suite task all passing |
| 46 | 1 | `DecisionGate.classify` (§11.1): nine deterministic need kinds + danger classification + model-assisted routing into the same bounded vocabulary; an invalid or failed tier classification returns `unknown`, never a guess; chatter never pays for classification (§4.6 regression guarded); the web need now *answers* instead of only opening a tab — live DuckDuckGo-lite results, dated and labelled as lookups (never memory), tier-summarized with a named best source, raw-plain without a tier, "couldn't reach" stated as such when the network fails; installs/uninstalls resolve the real package id first (`search_package` per platform parse-tested against real manager output: winget/brew/apt) — a search miss blocks auto-install ("no package matched — nothing installed, nothing guessed"), alternatives surface in the reply |
| 47 | 1 | `Navigator` (§11.3): vision-tier proposals executed through the authority gate one at a time, action budget, and three honest exits — `unplannable` (no action proposed = nothing clicked), `budget` (paused, not "done"), `needs_permission` (with the exact grant phrase); virtual + pyautogui limbs interchangeably; **accessibility eyes** (`accessibility.py`) read the live UI tree (AT-SPI/UIA) as bounded named text and click *buttons by name at real coordinates* — finds resolve case/fuzzy, misses are `target_not_found` (nothing clicked blindly), missing platform backends are declared gaps; the spoken imperative itself routes there ("log me in to X" → supervised navigator: gate first, budgeted actions, outcome narrated as unseated/needs-permission/completed/budget/failed — a stopped loop is never narrated as done); the longitudinal suite covers the full arc (`suites/gui_takeover.json` — ask → grant → complete → budget → continue-resume) |
...
| 50 | 1 | `webui.py` (§11.7): stdlib server + embedded dark chat page; dialog goes through the real `Mind.step` (labels, reminders, permission asks all over the wire — and pending asks render as Allow/Never buttons that send the real grant/deny sentence), state endpoint reports real counters, text-to-speech + speech-recognition via browser APIs; tested end-to-end over HTTP; `android_app/` WebView shell ships (Android Studio builds it; sandbox cannot compile it and says so); `Mind.organ_status()` + `--status` answer "what can you do right now" per organ, with the exact seat-it switch; a configured-but-down model tier is marked UNREACHABLE with the LM Studio fix instead of masquerading as seated `/api/state` also carries the overseer's day digest and itemised live queue (chips render in the header: today's trace count, paused/parked work) straight from the same Mind snapshot the conversation uses |
| 51 | 1 | Learning flow (§11.1) upgraded to real research: with a live tier the request finds the top YouTube source itself, reads its transcript (caption tracks — text the mind can honestly read), extracts cue-shaped steps deterministically, and the tier gap-checks the source ("what does this video NOT explain?") — gaps become recorded open questions, the reply names the source and says transcript-not-pixels; no captions = the vision gap stated, never faked; no tier = the task declared without a single invented step |
